...
export class HiveDataSync {
     ...
     */
    constructor(private userVault: HivePlugin.Vault, private showDebugLogs: boolean = false) {
        if (!userVault) {
     ...
     */
    public async deleteDatabaseEntry(contextName: string, key: string, deleteLocallyOnly: boolean = false) {
     ...
     */
    public async getDatabaseEntry(contextName: string, key: string): Promise<LocalBackupRestoreEntry> {
    ...
    private async upsertDatabaseEntryOnVault(contextName: string, localEntry: BackupRestoreEntry) {
        ...
        if (vaultEntry) {
            ...
            await this.userVault.getDatabase().updateOne(this.getCollectionNameForContext(contextName), {
                key: localEntry.key
            }, {
                $set: upsertedEntry
            });
     ...
     */
    public async sync(remotelyModifiedEntriesOnly: boolean = true): Promise<boolean> {
        ...
        for (let context of this.contexts) {
            ...
            this.saveLastFullSuccessfulSyncDate();
        }
    ...
    private async saveLastFullSuccessfulSyncDate() {
        return this.saveSettingsEntry("lastfullsyncdate", new Date().getTime());
    }

    private async clearLastFullSuccessfulSyncDate() {
